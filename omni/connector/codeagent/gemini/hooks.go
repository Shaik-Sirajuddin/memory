...
func readGlobalSettings() (map[string]string, error) {
	...
	if f.DefaultApprovalMode != "" {
		cfg["defaultApprovalMode"] = f.DefaultApprovalMode
	}
	if f.Sandbox != "" {
...
func writeGlobalSettings(cfg map[string]string) error {
	...
	f.Model = cfg["model"]
	f.ApprovalMode = firstNonEmpty(cfg["approvalMode"], cfg["defaultApprovalMode"])
	f.DefaultApprovalMode = firstNonEmpty(cfg["defaultApprovalMode"], cfg["approvalMode"])
...
func readSettingsFile(path string) (geminiSettingsFile, error) {
	...
	for k, v := range raw {
		...
		case "defaultApprovalMode":
			_ = json.Unmarshal(v, &f.DefaultApprovalMode)
		case "sandbox":
...
func writeSettingsFile(path string, f geminiSettingsFile) error {
	...
	if strings.TrimSpace(f.DefaultApprovalMode) != "" {
		raw["defaultApprovalMode"] = f.DefaultApprovalMode
	}
