...
-- MIN(rowid) preserves the first-created workspace; others get an 8-char hex tag.
-- Suffix collision probability is ~1/2^32 per pair; if it occurs the UPDATE re-runs
-- on the next DB open (ALTER TABLE is skipped as duplicate column, UPDATE is a no-op
-- for already-unique rows, CREATE INDEX retries) — fully self-healing.
UPDATE workspaces
...
-- succeed). On the next open, ALTER TABLE is skipped, UPDATE is a no-op, and
-- CREATE INDEX IF NOT EXISTS retries — recoverable with no manual intervention.
