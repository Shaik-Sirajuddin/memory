...
4. Connector maps `sandbox.Sandbox` to provider sandbox flag (`read-only` / `danger-full-access` for Codex)
...
The sandbox flag written to config is derived from the `sandbox.Sandbox` struct:
